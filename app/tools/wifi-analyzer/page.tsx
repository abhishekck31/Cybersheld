...
type WifiResult = { ok: boolean; title: string; details: string }
        ...
        <div className="text-center mb-6">
          <div className="flex justify-center mb-4">
            <div className="w-16 h-16 bg-indigo-700/20 rounded-full flex items-center justify-center border border-indigo-600/30">
              <Wifi className="h-8 w-8 text-indigo-300" />
            </div>
          </div>
          <h1 className="text-2xl font-bold">Wi‑Fi Security Scanner (Demo)</h1>
          <p className="text-sm text-gray-400">Enter an SSID to simulate a quick Wi‑Fi security overview.</p>
        </div>

        <Card className="bg-gray-800/50 border border-gray-700/50">
          <CardHeader>
            <CardTitle>SSID</CardTitle>
            <CardDescription>Enter a Wi‑Fi network name (SSID) to analyze</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              <Input value={ssid} onChange={(e) => setSsid(e.target.value)} placeholder="MyHomeWiFi" />
              <div className="flex space-x-2">
                <Button onClick={analyze} disabled={loading}>{loading ? "Scanning..." : "Scan"}</Button>
                <Button variant="ghost" onClick={() => { setSsid(""); setResult(null) }} disabled={loading}>Reset</Button>
              </div>
